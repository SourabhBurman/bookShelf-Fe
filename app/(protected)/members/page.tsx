...
import { PieChart, BookOpen, Users, IndianRupee, Settings, Search, Mail, ExternalLink } from "lucide-react";
...
const mockMembers = [
  { id: "M-101", name: "Alice Walker", activeRentals: 2, totalRentals: 15, fines: "$0.00", status: "Active" },
  { id: "M-102", name: "John Smith", activeRentals: 0, totalRentals: 3, fines: "$5.50", status: "Overdue" },
  { id: "M-103", name: "Emma Davis", activeRentals: 1, totalRentals: 8, fines: "$0.00", status: "Active" },
  { id: "M-104", name: "Michael Chen", activeRentals: 0, totalRentals: 42, fines: "$0.00", status: "VIP" },
];
...
export default function OwnerMembersPage() {
  ...
  return (
    <DashboardShell navItems={ownerNavItems} userTitle="Library Members">
      <div className="flex flex-col gap-8">
        ...
        <Card className="bg-zinc-900/50 border-white/5 backdrop-blur-xl overflow-hidden">
          <CardHeader className="border-b border-white/5 pb-4">
            <CardTitle className="text-white text-lg font-semibold">Registered Members</CardTitle>
          </CardHeader>
              ...
              <table className="w-full text-sm text-left">
                <thead className="text-xs text-zinc-400 uppercase bg-black/20">
                  <tr>
                    ...
                    <th className="px-6 py-4 font-medium hidden sm:table-cell">Total History</th>
                ...
                <tbody className="divide-y divide-white/5">
                  {mockMembers.map((m) => (
                    <tr key={m.id} className="hover:bg-white/5 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="h-10 w-10 rounded-full bg-purple-500/20 flex items-center justify-center shrink-0">
                            <span className="text-purple-400 font-semibold">{m.name.charAt(0)}</span>
                          </div>
                          <div>
                      ...
                      <td className="px-6 py-4">
                        <span className={m.fines !== "$0.00" ? "text-rose-400 font-medium" : "text-zinc-500"}>
                          {m.fines}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${
                          m.status === 'Overdue' ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' :
                          m.status === 'VIP' ? 'bg-purple-500/10 text-purple-400 border-purple-500/20' :
