...
function BooksExploreContent() {
  ...
  const books: Book[] = useMemo(() => {
    ...
    let mappedBooks = data.getBooks.map((b) => ({
      ...
      author: "Unknown Author", // API might not have author yet
      category: b?.genre || "Uncategorized",
      coverImage: `https://images.unsplash.com/photo-1544947950-fa07a98d237f?q=80&w=800&auto=format&fit=crop&random=${b?.id}`, // Mock unique cover
    ...
    return (
      <div className="flex flex-col items-center justify-center py-32 px-4 text-center">
        ...
        <p className="text-zinc-400 max-w-md mb-8">
          We had trouble fetching the catalog from our servers. Please try again or check back later.
          <br/>
          <span className="text-rose-400/80 text-sm mt-2 block break-all">Error: {error.message}</span>
        </p>
        <button
          onClick={() => getBooks()}
