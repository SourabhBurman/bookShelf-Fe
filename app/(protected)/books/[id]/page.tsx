...
import { Star, Library, MapPin, Share2, Heart, ShieldCheck, CheckCircle2, ChevronRight, Home, Clock, History } from "lucide-react";
...
const MOCK_REVIEWS = [
  { user: "Sarah Jenkins", avatar: "SJ", Initials: true, rating: 5, date: "Oct 12, 2023", text: "Absolutely loved this! The concept was mind-blowing and the execution was perfect." },
  { user: "David Miller", avatar: "DM", Initials: true, rating: 4, date: "Oct 05, 2023", text: "Great read. Pacing was a bit slow in the middle, but the ending tied everything beautifully." },
];
...
export default function BookDetailPage() {
  ...
  return (
    <DashboardShell navItems={navItems} userTitle="Book Details" userRoleLabel="Member">
      
      {/* Breadcrumbs */}
      <div className="flex items-center gap-2 text-sm text-zinc-400 mb-8 mt-2">
        <Link href="/dashboard/user" className="hover:text-white transition-colors">Home</Link>
        <ChevronRight className="h-4 w-4" />
        <Link href="/dashboard/user?category=Fiction" className="hover:text-white transition-colors">{MOCK_BOOK.category}</Link>
        <ChevronRight className="h-4 w-4" />
        <span className="text-white font-medium truncate max-w-[200px]">{MOCK_BOOK.title}</span>
      </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          <div className="relative aspect-[3/4] w-full rounded-2xl overflow-hidden shadow-2xl border border-white/10 group">
             ...
             <div className="absolute top-4 right-4 z-20 flex flex-col gap-3">
                ...
                <Button size="icon" variant="default" className="rounded-full bg-black/50 hover:bg-black/80 backdrop-blur-md border border-white/20 shadow-lg text-white">
          ...
          <div className="grid grid-cols-3 gap-2 bg-zinc-900/50 backdrop-blur-xl border border-white/5 rounded-xl p-4 divide-x divide-white/5 text-center">
             <div className="flex flex-col">
               <span className="text-zinc-500 text-xs font-semibold uppercase tracking-wider">Pages</span>
               <span className="text-white font-medium mt-1">{MOCK_BOOK.pages}</span>
             </div>
             <div className="flex flex-col">
               ...
               <span className="text-white font-medium mt-1 flex items-center justify-center gap-1">
                 {MOCK_BOOK.rating} <Star className="h-3.5 w-3.5 text-yellow-500 fill-yellow-500" />
               </span>
             </div>
             <div className="flex flex-col">
               <span className="text-zinc-500 text-xs font-semibold uppercase tracking-wider">Format</span>
           ...
           <div className="xl:col-span-7 flex flex-col pt-2">
             <div className="mb-4">
               ...
               <p className="text-xl text-zinc-400 font-medium">
                 by <span className="text-purple-300 hover:text-purple-200 cursor-pointer transition-colors underline underline-offset-4 decoration-purple-500/30">{MOCK_BOOK.author}</span>
               </p>
             </div>
             ...
             <div className="flex items-center gap-4 mb-8">
               ...
               <div className="text-sm text-zinc-400">
                 <span className="text-white font-medium">{MOCK_BOOK.rating}</span> ({MOCK_BOOK.reviewsCount.toLocaleString()} reviews)
               </div>
             </div>

             <div className="prose prose-invert prose-zinc max-w-none">
               <h3 className="text-xl font-semibold text-white mb-3">About this book</h3>
                   ...
                   <div key={idx} className="border-b border-white/5 pb-6 last:border-b-0 last:pb-0">
                     <div className="flex items-center justify-between mb-2">
                       <div className="flex items-center gap-3">
                         ...
                         <div>
                           <h4 className="font-medium text-white text-sm">{review.user}</h4>
                     ...
                     <p className="text-zinc-300 text-sm pl-11 leading-relaxed">{review.text}</p>
                   </div>
                 ))}
               </div>
             </div>
             ...
             <div className="mt-12">
               ...
               <ul className="space-y-3">
                 {MOCK_LIBRARIES.map((lib, idx) => (
                   <li key={idx} className="flex items-center justify-between p-4 rounded-xl border border-white/5 bg-zinc-900/30 hover:bg-zinc-800/50 transition-colors">
                      <div className="flex flex-col">
                        <span className="font-medium text-white">{lib.name}</span>
                        <span className="text-sm text-zinc-400">{lib.distance}</span>
                      </div>
                      <div>
                        {lib.available ? (
                ...
                <Card className="bg-zinc-900/80 border-white/10 backdrop-blur-xl shadow-2xl p-2 rounded-2xl overflow-hidden ring-1 ring-white/5">
                   ...
                   <CardContent className="p-6">
                     <div className="flex items-center justify-between mb-8">
                       <span className="text-zinc-400 font-medium">Format: Printed Book</span>
                     ...
                     <div className="flex flex-col gap-6">
                       ...
                       <label className="relative flex cursor-pointer rounded-xl border border-purple-500 bg-purple-500/10 p-4 shadow-sm hover:border-purple-400">
                         <input type="radio" name="purchaseType" className="sr-only" defaultChecked />
                         ...
                         <div className="flex flex-col">
                           <span className="text-zinc-300 text-sm font-medium uppercase tracking-wider mb-1">Rent Monthly</span>
                           <span className="text-3xl font-bold text-white">${MOCK_BOOK.rentPrice}<span className="text-sm font-medium text-zinc-500">/mo</span></span>
                         </div>
                       </label>
                       ...
                       <label className="relative flex cursor-pointer rounded-xl border border-white/10 bg-white/5 p-4 shadow-sm hover:border-white/20 transition-colors">
                         <input type="radio" name="purchaseType" className="sr-only" />
                         <div className="absolute top-4 right-4 h-5 w-5 rounded-full border-2 border-zinc-600 bg-zinc-900" />
                         <div className="flex flex-col">
                           <span className="text-zinc-400 text-sm font-medium uppercase tracking-wider mb-1">Buy Permanent</span>
                           <span className="text-3xl font-bold text-white">${MOCK_BOOK.purchasePrice}</span>
                         </div>
                       </label>
                     ...
                     <p className="text-center text-xs text-zinc-500 mt-4 flex items-center justify-center gap-2">
                       <Library className="h-4 w-4" /> Pick up available at 2 near libraries.
                     </p>
                   </CardContent>
                </Card>
             </div>
